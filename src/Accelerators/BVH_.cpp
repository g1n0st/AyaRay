...
namespace Aya {
	void BVHAccel_::construct(std::vector<SharedPtr<_Primitive> > prims) {
	...
	BBox BVHAccel_::worldBound() const {
		return m_root->m_box;
	}
	bool BVHAccel_::intersect(const Ray &ray, SurfaceInteraction *si) const {
		return intersect(m_root, ray, si);
	}

	bool BVHAccel_::intersect(BVHNode_ *node, const Ray &ray, SurfaceInteraction *si) const {
		bool is_leaf = false;
	...
	void BVHAccel_::construct(BVHNode_ **node, const int &L, const int &R) {
		if (L > R) {
			node = NULL;
			return;
		}
	...
	void BVHAccel_::freeNode(BVHNode_ **node) {
		if ((*node)->l_l != NULL) {
			freeNode(&(*node)->l_l);
		}
		if ((*node)->r_l != NULL) {
